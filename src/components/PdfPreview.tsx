...
import { FileSearch, Minus, Plus } from "lucide-react";
...
export default function PdfPreview({ file, pageCount = 0, selectedPages, status, position }: Props) {
  ...
  return <article className="card document-preview pdf-preview-card">
    <div className="preview-head">
      ...
      <div className="zoom"><button type="button" onClick={() => setZoom(Math.max(50, zoom - 10))} aria-label="Thu nhỏ"><Minus /></button><span>{zoom}%</span><button type="button" onClick={() => setZoom(Math.min(160, zoom + 10))} aria-label="Phóng to"><Plus /></button></div>
    </div>
    <div className="pdf-preview-body">
      {pageCount > 0 && <aside className="page-thumbnails" aria-label="Danh sách trang">{Array.from({ length: pageCount }, (_, index) => index + 1).map(page => <div key={page} className={selectedPages?.has(page) ? "page-thumb selected" : "page-thumb"}><span>{page}</span></div>)}</aside>}
      <div className="paper-viewport">
        ...
        {isPdf && <object data={preview} type="application/pdf" aria-label={`Xem trước ${file?.name}`} style={{ width: `${zoom}%` }} />}

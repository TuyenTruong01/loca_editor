...
type Clip = { file: File; duration: number; start: number; end: number };
...
export default function StudioPage() {
  ...
  const [currentTime, setCurrentTime] = useState(0); const [playing, setPlaying] = useState(false); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [result, setResult] = useState<Blob | null>(null);
  ...
  const urls = useMemo(() => clips.map(clip => URL.createObjectURL(clip.file)), [clips]);
  ...
  function importVideos(files: FileList | null) {
    ...
    const additions = Array.from(files).map(file => ({ file, duration: 0, start: 0, end: 0 }));
    setClips(previous => [...previous, ...additions]); setSelected(clips.length); setResult(null); setError("");
  }
  function updateDuration(duration: number) { setClips(previous => previous.map((clip, index) => index === selected ? { ...clip, duration, end: clip.end || duration } : clip)); }
  ...
  function removeClip(index: number) { setClips(previous => previous.filter((_, itemIndex) => itemIndex !== index)); setSelected(current => Math.max(0, Math.min(current, clips.length - 2))); setResult(null); }
  ...
  return <section className="studio">
    ...
    <div className="studio-grid"><aside className="media-panel"><div className="media-tabs"><button className="active"><Film />Video</button><button onClick={() => audioInput.current?.click()}><Music2 />Âm thanh</button></div><button className="import" onClick={() => videoInput.current?.click()}><Upload /> Nhập nhiều tệp</button><p>TỆP DỰ ÁN ({clips.length})</p><div className="media-list">{clips.map((clip, index) => <button key={`${clip.file.name}-${index}`} className={index === selected ? "media-item active" : "media-item"} onClick={() => { setSelected(index); setCurrentTime(0); }}><Film /><span><strong>{clip.file.name}</strong><small>{(clip.file.size / 1048576).toFixed(1)} MB · {formatTime(clip.duration)}</small></span><X className="remove-media" onClick={event => { event.stopPropagation(); removeClip(index); }} /></button>)}</div>{!clips.length && <div className="media-empty"><Film /><span>Chưa có tệp phương tiện</span></div>}</aside>
      <div className="studio-preview"><div className="screen">{active ? <><video key={urls[selected]} ref={videoRef} src={urls[selected]} onLoadedMetadata={event => updateDuration(event.currentTarget.duration)} onTimeUpdate={event => setCurrentTime(event.currentTarget.currentTime)} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onEnded={() => setPlaying(false)} />{texts.filter(item => currentTime >= item.start && currentTime <= item.end).map(item => <div className="video-text-overlay" key={item.id}>{item.text}</div>)}</> : <div><Play /><p>Nhập video để bắt đầu chỉnh sửa</p></div>}</div><div className="player"><span>{formatTime(currentTime)}</span><button disabled={!active} onClick={togglePlayback}>{playing ? <Pause /> : <Play />}</button><Volume2 /><input className="seek" type="range" min="0" max={active?.duration || 0} step="0.05" value={currentTime} onChange={event => { const value = Number(event.target.value); setCurrentTime(value); if (videoRef.current) videoRef.current.currentTime = value; }} /><span>{formatTime(active?.duration || 0)}</span></div></div>
    ...
    <div className="timeline"><div className="timeline-tools"><button disabled={!active} onClick={() => active && updateTrim("start", currentTime)}><Scissors /> Đặt điểm đầu</button><button disabled={!active} onClick={() => active && updateTrim("end", currentTime)}><Scissors /> Đặt điểm cuối</button><span>Timeline</span><b>{formatTime(totalDuration)}</b></div>
      <div className="track"><strong>Video</strong><div className="clip-row">{clips.map((clip, index) => <button key={index} className={index === selected ? "timeline-clip active" : "timeline-clip"} style={{ flexGrow: Math.max(1, clip.end - clip.start) }} onClick={() => setSelected(index)}><Film /> {clip.file.name}</button>)}<button onClick={() => videoInput.current?.click()}><Plus /> Video</button></div></div>

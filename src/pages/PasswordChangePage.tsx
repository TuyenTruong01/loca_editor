...
import { isPasswordRecovery, saveSession, updatePassword } from "../services/supabaseAuth";

export default function PasswordChangePage() {
  const { token } = useAuth(); const [password, setPassword] = useState(""); const [confirm, setConfirm] = useState(""); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  async function submit(event: React.FormEvent) { event.preventDefault(); if (password.length < 8) return setError("Mật khẩu phải có ít nhất 8 ký tự."); if (password !== confirm) return setError("Mật khẩu xác nhận không khớp."); setBusy(true); setError(""); try { const accessToken = await token(); if (!accessToken) throw new Error("Phiên đăng nhập đã hết hạn."); if (!isPasswordRecovery()) await markPasswordChanged(); await updatePassword(accessToken, password); saveSession(null); window.location.replace(`${window.location.origin}/video/studio`); } catch (reason) { setError(reason instanceof Error ? reason.message : "Không thể đổi mật khẩu."); } finally { setBusy(false); } }

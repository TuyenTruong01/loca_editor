...
export default function MergePdfPage() {
  ...
  async function addFiles(list: FileList | File[]) {
    setAdding(true); setError(""); setMessage(""); const existing = new Set(items.map(item => fingerprint(item.file))); const next: PdfItem[] = [];
    try {
  ...
  async function submit() {
    if (items.length < 2) return; setBusy(true); setError(""); setMessage("");
  ...
  return <section><div className="page-heading"><div><span className="eyebrow">DOCUMENT EDITOR</span><h2>Nối PDF</h2><p>Thêm nhiều file và sắp xếp đúng thứ tự trước khi nối.</p></div></div>
    <div className="document-layout pdf-tool-layout"><article className="card document-controls pdf-tool-controls"><div className="card-title"><span className="number">01</span><div><h3>Danh sách file PDF</h3><p>Thêm nhiều file và sắp xếp thứ tự trước khi nối.</p></div></div>
      ...
      <div className="merge-actions"><button type="button" onClick={() => input.current?.click()} disabled={adding}><FilePlus2 /> {adding ? "Đang thêm..." : "Thêm file PDF"}</button><button type="button" onClick={() => { setItems([]); setSelectedId(""); }} disabled={!items.length}><Trash2 /> Xóa tất cả</button></div>
      ...
      <button className="primary wide" disabled={items.length < 2 || busy || adding} onClick={submit}>{busy ? <><LoaderCircle className="spin" /> Đang nối PDF...</> : <><Merge /> Nối các file PDF</>}</button>

...
export default function DownloadPage() {
  ...
  async function startAudio() { if (!videoFile || audioBusy) return; audioTimer.start(); try { setAudioBusy(true); setMessage(""); setAudioResult(null); const blob = await extractAudio(videoFile); setAudioResult({ blob, name: `${videoFile.name.replace(/\.[^.]+$/, "")}.mp3` }); } catch (e) { setMessage(e instanceof Error ? e.message : "Không thể tách âm thanh."); } finally { setAudioBusy(false); audioTimer.stop(); } }
  return <section>
    ...
    <div className="split equal">
      ...
      <article className="card tool-card"><div className="card-title"><span className="number">02</span><div><h3>Tách âm thanh khỏi video</h3><p>Xuất phần âm thanh thành một tệp riêng.</p></div></div>
        <FileDrop file={videoFile} accept="video/*" hint="MP4, MOV, MKV, WebM hoặc AVI" onChange={file => { setVideoFile(file); setAudioResult(null); audioTimer.reset(); }} />
        {videoPreview && <video className="compact-video" src={videoPreview} controls />}
        <TimedActionButton label="Tách âm thanh" loadingLabel="Đang xử lý..." isRunning={audioTimer.isRunning} elapsedTime={audioTimer.formattedTime} disabled={!videoFile || audioBusy} onClick={startAudio} icon={<Headphones />} success={!!audioResult} error={!!message} />{audioResult && <div className="result"><span><strong>Âm thanh đã sẵn sàng</strong><small>{audioResult.name}</small></span><button className="primary" onClick={() => saveBlob(audioResult.blob, audioResult.name)}><Save /> Lưu tệp</button></div>}
      </article>
    </div>
  </section>;
}

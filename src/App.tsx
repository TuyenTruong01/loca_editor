...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="topbar"><button className="mobile-menu" onClick={() => setMenuOpen(!menuOpen)} aria-label="Mở trình đơn">{menuOpen ? <X /> : <Menu />}</button>{page !== "admin" && <nav className="top-tabs">{tabs[editor].map(tab => <button key={tab.id} className={page === tab.id ? "active" : ""} onClick={() => navigate(editor, tab.id)}>{tab.label}</button>)}</nav>}<div className="account-actions">{profile ? <><span><UserRound/><b>{profile.email}</b><small>{profile.role}</small></span><button onClick={() => void signOut()}><LogOut/> Đăng xuất</button></> : <button className="header-login" onClick={() => setShowLogin(true)}><UserRound/> Đăng nhập</button>}</div></header>
